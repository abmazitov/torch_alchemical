...
class SdLBFGS(Optimizer):
    ...
    def __init__(
        ...
        if len(self.param_groups) != 1:
            raise ValueError(
                "SdLBFGS doesn't support per-parameter options " "(parameter groups)"
            )
    ...
    def step(self, closure):
        ...
        orig_loss, inference_data = closure()
        loss = orig_loss.data
        current_evals = 1
        state["func_evals"] += 1
        ...
        while n_iter < max_iter:
            ...
            if state["n_iter"] == 1:
                ...
                for i in range(num_old):
                    ro[i] = 1.0 / old_stps[i].dot(old_dirs[i])
        ...
        return orig_loss, inference_data
