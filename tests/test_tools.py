...
def test_initialization_utilities():
    datamodule = LitDataModule(**datamodule_parameters)
    datamodule.prepare_data()
    datamodule.setup()
    ...
    assert torch.allclose(average_number_of_neighbors, torch.tensor(44.4383), atol=1e-4)
...
def test_litmodel():
    ...
    _ = LitModel(model=model, **litmodel_parameters)
